...
2026-09-25 說明文字更新：操作說明改為直接指示，並同步更新範例正文及註解。
論文、提案與書背重新完成編譯，50 項自動檢查通過、0 項失敗，
7 項人工檢查標記仍保留。已檢查三份 PDF 全文，並目視修改過的誌謝、
正文頁面及書背，未見缺字、裁切或排版溢出。
最新 PDF 雜湊、文字及量測結果已寫入 [validation-metrics.json](validation-metrics.json)。

2026-09-25 浮水印更新：改用 assets/CCU.jpg 原檔，
維持頁面置中、155 mm 寬及原有顯示頁面範圍。
論文與提案重新完成完整編譯；既有自動檢查共 44 項通過、0 項失敗，
6 項人工檢查標記仍保留。另逐頁確認論文第 3–13 頁、提案第 2–9 頁
嵌入的校徽像素與原圖一致，比例、位置及尺寸正確；其餘頁面無浮水印。
已目視論文誌謝與兩頁正文、提案目錄，文字、表格及公式仍可讀。

以下敘述保留為 **2026-09-24 的歷史驗證紀錄**；
其中 assets/watermark.png 的來源對應替換前版本。最新 PDF 雜湊與量測值
請以重新產生的 validation-metrics.json 為準。

驗證日期：2026-09-24。頁碼來源均指 format.pdf 的 **PDF 實體頁碼**。
需求判讀及三項排版設定見 [format-requirements.md](format-requirements.md)；
以下紀錄只適用於本次交付的佔位示例；正式論文完成後，請重新校對。
...
| 中文正文全形／表格及英文半形括號 | 3 | 範例內容、\ccucite、\ccueqref | 本次示例已人工校對；公式引用不跨行。新增文字後請重新校對 |
...
| 章後兩行；節前兩行、節後一行 | 1、3、17 | titlespacing、ccuchapterafter | 依模板設定，間距膠為 36／36／18 bp；目視及座標量測完成，基線差包含字型與 strut，非直接當空行高度 |
| 封面校名粗體36，其餘24，欄位排列 | 5、9 | \ccumakecover、thesis-config.tex | 依模板設定；所有欄位實測36或24 bp，中英題目、姓名、教授、民國年月順序及置中已目視 |
...
| 前置i起連續；正文1起至附錄 | 1，對照9–18 | pagenumbering roman／arabic | 模板設定已實作；主檔 i–viii、1–5；提案 i–iv、1–5 |
| 圖下圖號及斜體題、表上表號及斜體題 | 4 | caption、chapters/ 範例 | 圖／表順序、字級及對齊已量測與目視；圖框與圖題左界一致 |
| 註10點斜體、完整來源10點，置下 | 4 | \ccunote、\ccusource、\fullcite | 全部10.000 bp；註與來源靠左，表來源為完整書目而非僅列姓名與年份 |
...
| APA7 文獻樣式 | 6 | ccu-bibliography.sty、biblatex-apa | 已編譯與檢查書籍、期刊示例，年份、姓名、題名、卷期、頁碼、英文斜體及懸掛縮排正常；不能替所有文獻型別／真實性背書 |
| 文內英先中後，文末中先英後及筆畫／字母排序 | 6 | sorting=ccu、keywords／sortkey、\ccureferences | 亂序輸入後引文 Adams→Brown→王→林；文末王→林→Adams→Brown；請人工核對並維護中文排序碼 |
| 直排書背 | 6、8 | spine.tex、書背設定 | 一頁15×297 mm，級別→學校所別→文件名→題目→姓名→撰，無溢出；實體寬度需裝訂店確認 |
...
- **來源衝突的採用方式已實作**。請依格式要求文件第 2 節的設定排版；
  這是模板的採用方式，不能宣稱來源原本毫無矛盾。
